...
**本节要点:** `segment files` · `MemTuple, not HeapTuple` · `pg_appendonly` · `aux tables`
...
**本节要点:** `varblocks not pages` · `header family` · `two checksums` · `compression`
...
**本节要点:** `AOTupleId` · `segmentFileNum` · `rowNum` · `gp_fastsequence` · `GetFastSequences` · `block directory` · `AppendOnlyBlockDirectory_GetEntry` · `gp_aoblkdir()`
...
**本节要点:** `INSERT = append + commit-by-metadata` · `EOF-snapshot MVCC` · `DELETE/UPDATE as a visimap overlay` · `SCAN`
...
**本节要点:** `problem` · `compaction` · `states` · `threshold` · `demo`
...
**本节要点:** `The idea` · `File layout` · `Shared machinery` · `Per-column encoding`
