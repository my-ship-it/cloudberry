...
**本节要点:** `tables describing tables` · `pg_catalog schema` · `shared vs per-db` · `one catalog per segment` · `DDL is dispatched`
...
**本节要点:** `pg_class — one row per relation` · `pg_attribute — one row per column` · `pg_type — one row per type` · `pg_proc — one row per function` · `OID threads them all`
...
**本节要点:** `gp_segment_configuration = cluster map` · `gp_distribution_policy = per-table sharding` · `gp_id = per-node identity (the exception)` · `content −1 = coordinator` · `policytype p / r` · `internal.auto.conf carries gp_dbid`
...
**本节要点:** `pg_*.h CATALOG()` · `*.dat seed rows` · `genbki.pl` · `*_d.h OIDs` · `postgres.bki` · `Gen_fmgrtab.pl` · `fmgroids.h` · `initdb --boot` · `cdb_init.d`
...
**本节要点:** `syscache · `catcache.c`` · `relcache · `RelationData`` · ``pg_internal.init`` · `SI queue · `inval.c`` · `per-segment dispatch`
