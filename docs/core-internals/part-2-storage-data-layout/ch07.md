...
**本节要点:** `pg_am & friends` · `IndexAmRoutine` · `per-segment local` · `unique ⊇ distkey` · `TIDs`
...
**本节要点:** `meta(blk 0)→root→internal→leaf` · `Lehman-Yao B-link: high key + right-link` · `_bt_search descends · _bt_moveright` · `btbuild bottom-up · _bt_buildadd` · `_bt_doinsert → _bt_split` · `btgettuple vs btgetbitmap` · `btvacuumpage · half-dead · BTP_DELETED`
...
**本节要点:** `gist.h:30 GIST_CONSISTENT_PROC` · `gist.c:60 gisthandler` · `spgist.h:23 SPGIST_CONFIG_PROC` · `spgutils.c:43 spghandler` · `amcanorderbyop=true → kNN` · `point_ops · quad_point_ops`
...
**本节要点:** `item → many keys` · `entry tree (B-tree over keys)` · `posting list ⟵ short / posting tree ⟵ long` · `extractValue · extractQuery · consistent` · `@> ? @@ &&  (containment, not order)` · `fastupdate → pending list` · `jsonb @> '{…}' fast`
...
**本节要点:** `minmax summary per range` · `pages_per_range=128` · `revmap → summary tuple` · `lossy: candidate ranges + heap recheck` · `384 kB vs 43 MB btree` · `same idea as PAX sparse filters §6.4`
...
**本节要点:** `bitmap.c:74 bmhandler` · `BM_HRL_WORD_SIZE=64` · `LOV → bitmap vectors` · `BitmapAnd` · `low-cardinality OLAP`
