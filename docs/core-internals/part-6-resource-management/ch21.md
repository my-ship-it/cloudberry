...
**本节要点:** `two enforcement planes` · `the capability row` · `CGroupOpsRoutine vtable` · `declared, not probed` · `group DDL` · `slots and the FIFO queue` · `io_limit grammar` · `bypass valves` · `the views`
...
**本节要点:** `a slot is a lock` · `the live queue` · `six knobs, two tables` · `where admission happens` · `cost gating` · `statement_mem to operatorMemKB` · `the priority sweeper` · `the toolkit views` · `why legacy`
