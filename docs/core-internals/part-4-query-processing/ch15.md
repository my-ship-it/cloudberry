...
**本节要点:** `Query` · `rtable` · `jointree` · `targetList` · `PlannerInfo` · `RelOptInfo` · `pathlist` · `optimizer=off`
...
**本节要点:** `subquery_planner` · `pull_up_subqueries` · `reduce_outer_joins` · `deconstruct_jointree` · `process_equivalence` · `optimizer=off`
...
**本节要点:** `allpaths.c` · `costsize.c` · `pathnode.c` · `joinpath.c` · `add_path` · `cost_seqscan` · `standard_join_search` · `optimizer=off`
...
**本节要点:** `create_plan()` · `createplan.c` · `set_plan_references()` · `PlannedStmt` · `Motion seam → Ch16` · `Optimizer: Postgres`
