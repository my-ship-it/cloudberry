...
**本节要点:** `'Q' message` · `pg_parse_query` · `parse_analyze` · `planner` · `PortalRun` · `text→RawStmt→Query→PlannedStmt→rows`
...
**本节要点:** `Parse • Bind • Execute` · `exec_parse/bind/execute_message` · `prepared statements` · `pg_prepared_statements` · `portals` · `generic vs custom plans` · `EXPLAIN GENERIC_PLAN` · `\bind`
...
**本节要点:** `Slicing at Motions` · `SliceTable` · `Gangs` · `QE processes` · `serializeNode` · `DtxContext (snapshot+gxid)` · `cdbdisp_dispatchX` · `gp_command_count` · `Gather Motion`
