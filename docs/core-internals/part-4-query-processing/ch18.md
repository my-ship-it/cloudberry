...
**本节要点:** `volcano pull model` · `Plan ↔ PlanState` · `Start · Run · Finish · End` · `N+1 executors` · `EState / ExprContext` · `ExecProcNode trampoline` · `TupleTableSlot` · `rescan & squelch` · `EXPLAIN ANALYZE semantics`
...
**本节要点:** `Seq Scan` · `table AM` · `ExecScan loop` · `ring buffer` · `Index Only Scan` · `bitmap trio` · `Dynamic* family` · `Parallel Seq Scan`
...
**本节要点:** `tuplesort` · `Sort Method` · `statement_mem` · `spill` · `gp_workfile_*` · `HashAggregate` · `hash_mem_multiplier` · `two-stage agg` · `Streaming` · `WindowAgg` · `TupleSplit`
...
**本节要点:** `nodeNestloop.c` · `nodeMergejoin.c` · `nodeHashjoin.c` · `nodeHash.c` · `batches` · `Memoize` · `prefetch_inner`
...
**本节要点:** `execExpr.c` · `ExprEvalStep` · `EEOP_ opcodes` · `computed goto` · `ExecQual / ExecProject` · `execGrouping` · `SRF / ProjectSet` · `JIT status`
...
**本节要点:** `InitPlan vs SubPlan` · `hashed SubPlan` · `Materialize` · `WITH RECURSIVE` · `Append / MergeAppend` · `Limit / Unique / SetOp` · `ModifyTable`
...
**本节要点:** `Split Update` · `Split Merge` · `Assert` · `Shared Scan` · `Sequence` · `Partition Selector` · `RuntimeFilter` · `Table Function`
