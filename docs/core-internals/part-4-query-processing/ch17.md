...
**本节要点:** `the gate` · `one-time init` · `optimize_query` · `into C++` · `fallback`
...
**本节要点:** `memo` · `CGroup` · `CGroupExpression` · `CEngine::Optimize` · `jobs and scheduler` · `enforcers` · `search stages` · `plan space`
...
**本节要点:** `CXform` · `pattern` · `binding` · `xform families` · `optimizer_print_xform` · `join-order enumerators` · `decorrelation` · `disable_xform`
...
**本节要点:** `CMDProviderRelcache` · `CTranslatorRelcacheToDXL` · `IMDRelation` · `CMDAccessor` · `CMDCache` · `MDCacheNeedsReset` · `pg_statistic → DXL` · `damping factors` · `missing stats` · `CCostModelGPDB` · `CostMotion` · `optimizer_segments` · `cost scales`
...
**本节要点:** `the sandwich` · `libgpos/libgpopt/libnaucrates` · `Var → colid` · `unsupported features` · `EXPLAIN (DXL on)` · `colid → Var` · `slice table` · `minidump`
...
**本节要点:** `CConfigParamMapping` · `traceflags` · `xform-disable bits` · `optimizer_enable_*` · `optimizer_control` · `optimizer_segments` · `plan hints` · `pg_hint_plan` · `print GUCs` · `minidump` · `playbook`
