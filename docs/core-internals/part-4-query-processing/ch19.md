...
**本节要点:** `a Motion is an operator` · `slices & gangs` · `no global coordinator` · `sender/receiver duality` · `partitioned · replicated · singleton` · `reading the N:M label`
...
**本节要点:** `gang` · `writer vs reader` · `gpqeid` · `QE reuse` · `poll() loop` · `'M' payload` · `QE death`
...
**本节要点:** `PlanSlice` · `ExecSlice` · `SliceTable` · `processesMap` · `slice numbering` · `rootIndex` · `InitPlan slices`
...
**本节要点:** `one node, two roles` · `the sender path` · `four motion types` · `cdbhash routing` · `TupleChunk byte layout` · `the transport picks the chunk size` · `ChunkSorterEntry + htupfifo` · `tupleremap` · `Merge Key` · `stop and EOS` · `what EXPLAIN really prints`
...
**本节要点:** `MotionIPCLayer vtable` · `self-registration` · `dynamic GUC enum` · `setup / teardown` · `UDPIFC packet` · `sliding window` · `where a sender blocks` · `ic-tcp` · `ic-proxy` · `ic_bench`
...
**本节要点:** `DECLARE … PARALLEL RETRIEVE CURSOR` · `un-gathered plan` · `endpoint` · `auth token` · `retrieve-mode connection` · `RETRIEVE ALL FROM ENDPOINT` · `endpoint states` · `gp_wait_parallel_retrieve_cursor`
