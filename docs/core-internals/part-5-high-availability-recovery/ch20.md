...
**本节要点:** `insert path` · `rmgr table` · `WAL writer / flush` · `segments & archiving` · `xact records` · `checkpoints` · `distrib commit/forget`
...
**本节要点:** `walsender / walreceiver` · `connect & START_REPLICATION` · `synchronous replication` · `FTS syncrep-off`
...
**本节要点:** `StartupXLOG / DBState` · `redo loop & invalid pages` · `crash vs archive fsync` · `distributed 2PC recovery` · `prepared-xact survival` · `standby promotion`
...
**本节要点:** `process model & GUCs` · `probe protocol` · `non-blocking poll cycle` · `state machine` · `catalog & version bump` · `mirror promotion` · `standalone gpfts + etcd`
...
**本节要点:** `the fault` · `detection & retries` · `catalog role-flip` · `promotion` · `reroute & 2PC` · `gprecoverseg`
...
**本节要点:** `FTS & recovery GUCs` · `xact WAL opcodes` · `FTS response fields` · `DBState values`
