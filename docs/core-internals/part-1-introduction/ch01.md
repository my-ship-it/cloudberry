...
**本节要点:** `shared-nothing` · `distribution policies` · `cdbhash → segment` · `cluster catalogs`
...
**本节要点:** `postmaster tree` · `QD vs QE` · `background workers` · `shared memory`
...
**本节要点:** `wire protocol` · `QD→QE dispatch` · `gangs & slices`
...
**本节要点:** `the module map` · `reading the map` · `chapter guide`
