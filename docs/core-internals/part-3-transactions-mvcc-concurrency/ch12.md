...
**本节要点:** `why atomic commit` · `the two phases` · `the blocking problem` · `PG's participant role`
...
**本节要点:** `the DTM on the QD` · `DtxState commit spine` · `one-phase optimization` · `QE DtxContext roles`
...
**本节要点:** `the commit point` · `atomic cross-segment visibility` · `distributed snapshot decides` · `distributedlog mapping`
...
**本节要点:** `in-doubt after crash` · `the dtx recovery process` · `commit-log decides verdict` · `local↔distributed mapping`
